...
-- 
-- Version 1.0.19 -- Vanilla/Turtle WoW version
...
-- What version is this? (Vanilla/Turtle WoW specific)
VgerCore.IsTurtle = true
VgerCore.IsClassic = true
VgerCore.IsBurningCrusade = false
VgerCore.IsWrath = false
VgerCore.IsCataclysm = false
VgerCore.IsMainline = false
VgerCore.IsWarWithin = false
VgerCore.IsDragonflight = false
VgerCore.IsShadowlands = false

-- Feature flags for Vanilla
VgerCore.DeathKnightsExist = false
VgerCore.MonksExist = false
VgerCore.DemonHuntersExist = false
VgerCore.EvokersExist = false
VgerCore.SpecsExist = false
VgerCore.RangedSlotExists = true
VgerCore.ArtifactsExist = false
VgerCore.EquipmentSetsExist = false
VgerCore.ReforgingExists = false
...
-- Outputs a message as-is to the default chat window.
function VgerCore.Message(Message)
	-- In Lua 5.0, we need to be careful with nil values
...
-- Outputs a message in white to the default chat window.
function VgerCore.MultilineMessage(Message)
	if not Message then Message = "(nil)" end
	VgerCore.Message(VgerCore.Color.White .. tostring(Message))
end

-- Fails an assertion and writes a message if a condition is false.
...
function VgerCore.HookInsecureFunction(arg1, arg2, arg3, arg4)
	...
	elseif TypeOfObject == "string" then -- FunctionName, Hook, Pre
		...
		if OldFunction then
			setglobal(arg1, VgerCore.CreateHookFunction(OldFunction, arg2, arg3))
		else
...
function VgerCore.CreateHookFunction(OldFunction, Hook, Pre)
	if Pre then
		return function(a1, a2, a3, a4, a5, a6, a7, a8, a9, a10)
			...
			return OldFunction(a1, a2, a3, a4, a5, a6, a7, a8, a9, a10)
		end
	else
		return function(a1, a2, a3, a4, a5, a6, a7, a8, a9, a10)
			local r1, r2, r3, r4, r5 = OldFunction(a1, a2, a3, a4, a5, a6, a7, a8, a9, a10)
			Hook(a1, a2, a3, a4, a5, a6, a7, a8, a9, a10)
...
function VgerCore.ExecuteChatCommand(Command)
	...
	if SlashCommand then
		if IsSecureCmd and IsSecureCmd(SlashCommand) then
...
function VgerCore.RunMacro(MacroName)
	...
	local Line
	-- In Lua 5.0, use string.gfind instead of string.gmatch
...
function VgerCore.IsInBattleground()
	local Battleground
	for Battleground = 1, MAX_BATTLEFIELD_QUEUES or 3 do
		local status = GetBattlefieldStatus(Battleground)
		if status and status == "active" then return true end
	end
	return false
end
...
function VgerCore.FormatShortDecimal(Number)
	...
	elseif Number >= 1000 then
		-- BreakUpLargeNumbers already implemented in PawnCompat_Vanilla.lua
...
-- Throttle and Delay functions need C_Timer compatibility from PawnCompat_Vanilla.lua
...
-- Lua 5.0 compatibility helper
function VgerCore.TableLength(t)
	return table.getn(t)
end
